...
6. Faça `poll` do processo usando o `session id` existente até ele terminar; não envie conteúdo adicional à sessão. O guardian abre request e task sem seguir links, confere ambos contra o manifesto e cria snapshots imutáveis limitados. Antes do spawn, remove os plaintexts por descritor. Entrega a task por `stdin=PIPE` com EOF e o request por descritor anônimo interno `--request-fd`, sem reabrir path compartilhado; o launcher propaga o exit status do filho.
