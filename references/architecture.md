...
No Windows, onde `fcntl`, `dir_fd`, `pass_fds` e sinais POSIX não existem, o runtime usa uma trilha de compatibilidade: locks passam por `msvcrt.locking`, o guardian valida os paths por nome, remove os plaintexts originais antes do spawn, grava o request em um snapshot temporário privado passado por `--request`, remove esse snapshot no `finally` e tenta encerrar árvore com `taskkill /T` antes do fallback do Python. Essa trilha é compatível com Codex no Windows, mas a garantia de request anônimo unlinkado é exclusiva do POSIX.
